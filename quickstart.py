"""
Lumynax Reranker Bge V2 M3 — LumynaX quickstart (clone & run, multimodal safetensors).
...
def main():
    ...
    if not (HERE / "model.safetensors").exists():
        ...
        print(f"[lumynax] run: hf download AbteeXAILab/lumynax-reranker-bge-v2-m3 --local-dir <dir> first.", file=sys.stderr)
        sys.exit(2)
